...
const areTeamMembers = (team, userIds) =>
  userIds.every((userId) => isTeamMember(team, userId));

const canAccessTeam = (req, team) =>
  req.user.role === "Admin" ||
  getId(team.createdBy).toString() === req.user.id.toString() ||
  isTeamMember(team, req.user.id);
...
export const createTask = async (req, res) => {
  try {
    ...
    if (!canAccessTeam(req, project.team)) {
      return res.status(403).json({ message: "You are not a member of this project's team" });
    }

    if (assignedTo && !areTeamMembers(project.team, assignedTo)) {
...
export const updateTask = async (req, res) => {
  try {
    ...
    if (!canAccessTeam(req, task.project.team)) {
      return res.status(403).json({ message: "You are not a member of this project's team" });
    }
    ...
    if (assignedTo) {
      if (!areTeamMembers(task.project.team, assignedTo)) {
